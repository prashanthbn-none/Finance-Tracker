# RupeeFlow - India Finance Tracker

A web-based INR-first finance tracker built with React + Vite. Manage income,
UPI spends, EMIs, budgets, savings goals, recurring bills, bank statement
imports, reports, and analytics. Data is stored locally in the browser.

## Features

- **Authentication** — register, login/logout, email OTP password reset, profile (accounts remain local)
...
- **Transaction history** — search; filter by type, category, date range, and amount; sort; bulk select & delete; PDF statement import; CSV/Excel export
...
- **Settings** — theme, INR display, data export, JSON backup & restore
...
```bash
npm install
npm run dev      # start app + OTP email server (usually http://localhost:5173)
...
Copy `.env.example` to `.env` and fill the SMTP values. For Gmail, enable
two-step verification and use a Google App Password in `SMTP_PASS`; never use
your normal Gmail password. OTP codes expire after 10 minutes, allow five
attempts, enforce a 60-second resend delay, and issue one-time reset tokens.
